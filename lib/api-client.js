...
class ApiClient {
  ...
  triggerBlueprint(blueprintId, teamId, users, channel, context) {
    return this._doRequest({
      method: 'POST',
      endpoint: `teams/id/${teamId}/triggers`,
      body: {
  ...
  triggerBlueprintRevision(blueprintRevisionId, teamId, users, channel, context) {
    return this._doRequest({
      method: 'POST',
      endpoint: `teams/id/${teamId}/triggers`,
      body: {
        blueprint: {revision: {id: blueprintRevisionId}},
        users: users,
        channel: channel,
        context: context,
      }
  ...
  updateWhitelistedDomains(channelId, platform, whitelistedDomains) {
    return this._doRequest({
      apiVersion: '2.0',
      method: 'PUT',
      endpoint: `channels/${platform}/${channelId}/whitelisted-domains`,
  ...
  searchUser(channelId, platform, query) {
    return this._doRequest({
      apiVersion: '2.0',
      method: 'POST',
      endpoint: `channels/${platform}/${channelId}/users/search`,
      body: query,
    });
  ...
  addTeamChannel(teamId, channel) {
    return this._doRequest({
      apiVersion: '2.0',
      method: 'POST',
      endpoint: `teams/id/${teamId}/channels`,
      body: channel
    });
  }

  addTeamEvent(teamId, event) {
    return this._doRequest({
      apiVersion: '2.0',
      method: 'POST',
      endpoint: `teams/id/${teamId}/events`,
      body: event
    });
  }
}
...
class StagingApiClient extends ApiClient {
    constructor(accessToken) {
        super(accessToken, 'https://api.staging.maxwell.ai/')
    }
}
